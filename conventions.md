...
4. **Single source of truth per fact.** Default prompts: `DEFAULT_PROMPT_TEMPLATES` only. Slot parsing: `extractSlotNames` only. Prompt splitting: `splitPrompt` only. Message shapes: `contracts.ts` only. Duplicating any of these (even in UI "for convenience") is the bug class that caused ARCH-01/ARCH-03 — don't reintroduce it.
