...
Hard property: **≤ 3 Anthropic calls per generate/refine invocation** — initial + one repair + one RESHAPE (single mode: the classic tighten, the N=1 case; thread mode: tighten-the-named-posts and/or a count nudge when the post count drifts beyond ±1 of the ≈N target — folded into the repair when both fire). Thread invocations parse the `---` wire format (lib/thread) and run every deterministic pass PER POST, with 4096 max output tokens (singles stay at 1024). Refine is the same pipeline minus sampling — it reshapes `previousDraftText` from the panel (threads travel joined). Every call renders through `assembleRefinePrompt` (the one refine template) and so carries the same system voice anchor as generation; repair/tighten/reshape/repack get code-supplied instructions from `lib/prompt/defaults`.
...
| IndexedDB                | db `x-post-composer`, `DB_VERSION = 6` | store `items`: `LibraryItem` rows (threads carry `segments`), keyPath `id`, index `byType` · store `bundles`: `Bundle` rows, keyPath `id` | persistent              |
