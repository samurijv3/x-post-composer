...
2. In Compose, the **“Sounding like …”** line under the composer picks the voice seed: choose a bundle and its members become the _exact_ voice examples for that generation — no sampling, no topping up. Starred examples still ride on top. The inspector labels the call `generate (bundle: …)`. (Before you have any bundles it shows the default library sample and points you at Voice.)
