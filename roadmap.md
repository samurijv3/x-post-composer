...
Field comparison against x.com found the panel's draft text reading larger than X. Audit verdict: every type size matched its X counterpart except the draft's 17px — the handoff's deliberate "focal tweet" size. The theory didn't survive contact: the draft wears TIMELINE anatomy (40px avatar, 15px name row) directly beside X's 15px timeline, in a ~400px column where 17px wraps at ~38 characters — it read inflated, not focal. The draft now uses X's timeline metrics (15px/1.45), agreeing with thread segments, context tweets, and library rows; focality comes from position, whitespace, and the ring. One editor metric now serves every surface (the per-context overrides collapsed); `--fs-lg` stays reserved with the correction noted.
