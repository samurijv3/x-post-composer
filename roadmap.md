...
### 2026-06-12 — the capture bundle target survives mode toggles

"Mode and target live and die together" (a Phase 6 call) turned out to fight the actual workflow: browsing X for examples to feed one bundle means flipping capture off and on constantly, and re-picking the bundle on every flip was the friction. The target now persists across toggles within the browser session (still session storage — quitting the browser clears it; a deleted bundle still resets it; the background still validates at capture time). Honesty holds without the clearing: the target is visible in the banner whenever the switch is on, and every save states "Filed into …". _Refined same day:_ the lifetime is "while the panel is open", not the whole browser session — the background clears the target when the LAST panel closes (the existing port-disconnect seam), so reopening the extension always starts at plain library; only toggles within one panel sitting keep the choice.

### 2026-06-12 — the highlights explain themselves (the violation note)

Residual-violation highlights carried no explanation — the model got a clear account of what broke (`summarizeViolations` in the repair instruction) while the human got unexplained marks. Now a `ViolationNote` renders with the marks: which of YOUR rules broke (`lib/exclusion describeViolations` — counted structural rules, banned words quoted by entry; a deliberate sibling of the model-facing summary, never shared, since UI and prompt copy drift for different reasons), that a repair pass already ran (true by construction — residuals only exist after it), and both exits (hand-edit clears the marks / regenerate). Placement: the over-cap callout's slot for singles, under each affected card for threads — violations are per-post. Per-mark hover tooltips were ruled out structurally: the marks live in an aria-hidden backdrop BEHIND the textarea, which owns all pointer events; the note also happens to be the first time violations are announced to screen readers at all.

### 2026-06-12 — arrow-key navigation in capture modes (↑/↓ + Enter)

While a capture mode is armed, ↑/↓ step the hover highlight tweet by tweet and Enter selects — the keyboard drives the SAME cursor and the same capture path as the mouse, nothing parallel. Boundary call: intercepting keys while a mode is on is the same carve as the existing click interception (an explicitly-armed, visibly-indicated mode changes how input acts on the page); scrolling the cursor into view is display, not action — no posting, no navigation, no DOM mutation of X's tree, so §6 stands untouched. Hard guards: never while focus is in an editable field (Enter must never be stolen from a composer), never with modifiers held. Two mechanics worth recording: (1) at the rendered list's edge the press deliberately passes through, so the native scroll makes X's virtualization render more tweets and the next press continues — degrade-by-design instead of fighting the virtual list; (2) Chrome re-fires mouseover when our scrollIntoView slides the page under a stationary pointer, which would snap the cursor straight back to the mouse — a short unmoved-pointer suppression window after each key step keeps the keyboard in charge while real mouse movement still takes over instantly. The stepping brain (`stepToAdjacentArticle`) is fixture-tested: DOM-order walk, modal-layer confinement, viewport-anchored start, edge nulls.

### 2026-06-12 — keyboard nav refined against Undercurrent's list nav (lifted patterns)

The first cut worked but was unrefined; the user's Undercurrent project had already polished this interaction class, so its patterns were lifted where they fit. Taken: (1) **a keyboard-nav mode flag instead of a time window** — while the keyboard drives, hover events don't move the cursor AT ALL, and only genuine pointer movement (coordinates changed — scroll-end synthetic mousemove keeps the old coords) hands control back; this replaces the previous 600 ms unmoved-pointer suppression window, removing the arbitrary timeout and covering smooth scrolls of any duration. (2) **Margin-based smooth nudge scrolling** — scroll only when the cursor article enters the 100px edge bands, only BY the overshoot, smoothly; mid-viewport steps don't scroll at all (the old scrollIntoView recentered on every step — the jumpiness was most of the "unrefined" feel). Adaptation for tall content: a taller-than-band tweet anchors its top. (3) **Held-key acceleration** — repeat count ramps the step 1→2→3, clamped at the rendered ends; capped far below Undercurrent's ×8 because tweets are tall where its table rows are dense. (4) **Nav resumes from the selection** — in reply-context mode a fresh cursor starts from the locked tweet when rendered, not the viewport top. Deliberately NOT lifted: j/k (X owns those keys natively on its own page — shadowing them invites two competing highlights), g-chords for top/bottom (an infinite virtualized feed has no bottom), selection-follows-highlight (arrowing the LOCK would fire new-context per step and shred the workbench), and mount-time blur of the focused element (acting on X's page state).

### 2026-06-12 — Escape de-selects; nav keys forwarded from the panel

Two field follow-ups to the keyboard nav. **Escape de-selects in stages**: the first press drops the highlight cursor, the next clears the reply-context lock (sharing the overlay ×'s `dismissReplyContext` path — optimistic local clear, authoritative push back). While X has a modal open, Escape stays X's (it closes the modal); the staged order means Esc never destroys the lock by surprise while a cursor is showing. **The focus gap**: keystrokes land in the focused document, and right after arming a capture mode that's the PANEL — so arrows were dead until the user clicked the X page once, despite the hover outline inviting navigation. Rejected fix: stealing window focus on hover (would hijack typing in the panel's brief box mid-drift). Built fix: the panel forwards plain ↑/↓/Enter/Esc (`panel:capture-nav` → background → `bg:capture-nav` → the ACTIVE X tab only — stepping cursors on background tabs would scroll pages the user isn't looking at) into the same `handleCaptureNavKey` the page's own listener uses, with `repeat` carried so held-key acceleration works from both sides. Panel guards mirror the page's (no modifiers, never from editable/interactive elements) with one exemption: the capture toggle's own checkbox — exactly where focus sits after arming — since arrows/Enter have no native job on it.

### 2026-06-12 — nav field pass: acceleration reverted; Escape gated on visibility

Two corrections from the field. **Acceleration out**: the 1→2→3 held-key ramp (lifted from Undercurrent) doesn't transplant — its rows are short and its scroll container small, while tweets are tall and the page's smooth scroll can't keep repeat pace, so the cursor escaped the viewport. Replaced with consistent one-article steps plus a scroll-behavior split: discrete presses glide (smooth), held repeats scroll INSTANTLY — controlled beats fast, and the cursor can now never leave the screen. **Escape's stages gate on visibility, not existence**: after Enter locks a tweet, the cursor still points at it but paints nothing (hovered === locked suppresses the preview), so the first Esc cleared an invisible cursor — a visually dead press, reported as "I have to press it twice." A cursor only counts as a stage when clearing it changes something on screen; an invisible or stale cursor clears silently along with the lock in one press.

### 2026-06-12 — the resume anchor: the first press after a release re-acquires, the second moves

Field bug: Enter (lock) → Escape (release) → ↓ outlined the tweet ABOVE the released one (↑ the one below). Cause: a cleared cursor kept no memory, so the next press fell back to the viewport-anchored initial pick — whose "first article in view" is usually the partially-visible one above the user's actual place. Fix: `lastCursorArticle` remembers wherever the cursor last sat (keyboard- or mouse-placed) and survives the cursor clearing; the first arrow press with no live cursor re-acquires that exact tweet — consuming the press — and only subsequent presses step. The anchor yields when stale: disconnected (virtualization unmounted it) or out of the active layer falls back to the existing picks (lock, then viewport). This also fixes the same confusion for the mouse paths nobody had named yet: drifting off articles or out of the window then pressing an arrow now returns to the last hovered tweet instead of viewport-guessing.

### 2026-06-12 — the X-native reskin (design_handoff_x_native_reskin, all seven phases)

The full visual reskin shipped per the handoff spec: X-native tokens (pure white / Dim #15202B / Lights Out #000, the one #1D9BF0 accent in every theme), hairlines over cards, two radii, pill buttons (blue compose-side, monochrome Voice/settings-side), the draft as a real tweet (avatar, @handle · draft, 17px text, count ring), the reply composer on the avatar rail, the "Sounding like …" seed line + popover, thread segments as X's self-thread anatomy, Voice as a full-bleed library, labeled tabs, the blue X toast, single-column X-style settings with the four-way theme row, and the overlay re-skinned (page-background luminance theme detection, inside-the-rect blurred dismiss, the "Replying in Margin" pill). Spec-silent judgment calls, per the handoff's rule 7: `--accent-press #177cc0` (one step past hover); weak tone tokens survive for the notice discs; `--warn` brightens on dark themes (#B8860B is illegible on #15202B); the type-scale variables keep their names with new values. Deliberate deviations recorded: the Voice "+ Add" sits at the filter row's end rather than the panel header (the header stays screen-agnostic; revisit if felt); Expand/Collapse-all was RETAINED as an icon (the mockup omits it, but deleting a working affordance is product change, not reskin); the capture row's filing select stays a native select restyled inline (not a popover); the bundle ghost-row's ×-removes-the-hole upgrade is deferred. Theme storage extended to auto|light|dim|lights with stored 'dark' reading as Dim (read-side only) and 'auto' resolving via prefers-color-scheme — the one sanctioned non-visual change, alongside overlay.ts per the spec. Legacy token aliases were introduced in phase 1 and fully burned down in phase 7 (zero oklch, zero legacy var() uses remain). All copy changes were the specified set; every other string kept verbatim. Acceptance greps pass code-side; the pixel pass against x.com in all three themes needs the live browser.

### 2026-06-12 — reskin field pass: the draft is timeline-sized

Field comparison against x.com found the panel's draft text reading larger than X. Audit verdict: every type size matched its X counterpart except the draft's 17px — the handoff's deliberate "focal tweet" size. The theory didn't survive contact: the draft wears TIMELINE anatomy (40px avatar, 15px name row) directly beside X's 15px timeline, in a ~400px column where 17px wraps at ~38 characters — it read inflated, not focal. The draft now uses X's timeline metrics (15px/1.45), agreeing with thread segments, context tweets, and library rows; focality comes from position, whitespace, and the ring. One editor metric now serves every surface (the per-context overrides collapsed); `--fs-lg` stays reserved with the correction noted. _Same day:_ the composer input followed (spec'd 18px → 15px) — input and output reading at two different sizes felt off once the draft shrank.

### 2026-06-12 — reskin field pass: Voice section hierarchy restored

Two field corrections to the phase-4 Voice layout. (1) Bundle rows kept their pre-reskin card chrome, giving "Day X series" the same visual rank as the Bundles header it belongs under — they're now de-carded, hairline-divided children: indented under the header, name one size down, members indented further. (2) The mockup dropped the "Saved examples" header entirely (the tab as the title) — field-falsified: a collapsed bundle row followed directly by library rows reads as if the rows were the bundle's children. The collapsible Saved examples header returns (mirroring Bundles' pattern, carrying the + add and expand-all actions that had been parked in the filter row), and the locate flash re-opens it, restoring the pre-reskin guarantee that "Show me" never lands in a collapsed section.

### 2026-06-12 — filter chips combine: ★ + one type via ⌘/Ctrl-click

The Voice filter becomes two axes mirroring the data model — type (exclusive: all/posts/replies/threads) and the orthogonal star flag — replacing the single five-value enum where ★ was a fake fifth type. Plain click keeps the familiar exclusive-select (resets the other axis); ⌘/Ctrl-click changes only the clicked axis, so Threads then ⌘-★ shows starred threads; re-⌘-clicking an active value toggles it off. Only ★+type combos exist by construction — types stay mutually exclusive. Faceted counts generalize: each chip counts search ∩ the OTHER axis (with ★ on, "Posts 3" means starred posts; with Threads on, "★ 2" means starred threads). Discoverability rides the tooltips; the stuck-filter resets and the locate widen cover both axes.
