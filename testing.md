...
Current state: **362 tests, 24 files**, all green via `npm run test`.
...
| Sampling                                                       | `src/lib/sampling/selectExamples.test.ts`                | The §8 seam — Concept A: guaranteed stars, the cap, tier fill/top-up, no-duplicates, empty edges (injected RNG)                                                      |
...
| Corpus + IDB migration                                         | `src/storage/corpus.test.ts`                             | CRUD, duplicate-id constraint, single-tx clear, and real seeded migrations (v1→v3 chain, v2→v3 taxonomy collapse)                                                    |
